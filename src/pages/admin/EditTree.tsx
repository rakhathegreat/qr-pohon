...
const EditTree: React.FC = () => {
  ...
  const [form, setForm] = useState<Tree | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form) return;
    setSaving(true);
    const { error } = await supabase.from('trees').update(form).eq('id', id!);
    setSaving(false);
    if (!error) nav('/');
  ...
  if (!form) return <p className="p-6">Tree not found</p>;
